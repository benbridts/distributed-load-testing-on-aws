...
if [ "$TEST_TYPE" != "simple" ]; then
  ...
  if [ -f results.txt ]; then
    ...
    for f in "${files[@]}"; do
      ...
      echo "Uploading $p"
      aws s3 cp $f $p --region $MAIN_STACK_REGION || true  # Don't fail the build if a file upload fails
    done
  fi
fi
...
if [ -f /tmp/artifacts/results.xml ]; then
  ...
  aws s3 cp /tmp/artifacts/bzt.log s3://$S3_BUCKET/results/${TEST_ID}/bzt-${PREFIX}-${UUID}-${AWS_REGION}.log --region $MAIN_STACK_REGION || true  # Don't fail the build if the log upload fails
  if [ -z "$LOG_FILE" ]; then

...
- Patched libssl in the web-console image to address vulnerabilities.
- Updated the load-tester base image to a newer Amazon Linux 2023 digest.
- Patched the browserslist npm package vulnerability.

## [4.2.3] - 2026-08-19

### Security

- Patched the brace-expansion npm vulnerability by updating aws-cdk-lib in metrics-utils and infrastructure.
- Upgraded the web-console image python3 to address vulnerabilities.
- Updated the load-tester base image to a newer Amazon Linux 2023 digest, patching wget (CVE-2026-58469).
...
- **DevOps Agent description limit**: Truncated investigation descriptions to the AWS DevOps Agent size limit.

### Security

- Upgraded npm packages for security patching (postcss, react-router, esbuild via vite, curl)
...
- Added missing package.json/lock from root directory

### Security

- Upgraded npm packages for security patching
- Upgrade web-console image library for security patching

## [4.1.1] - 2026-06-01

### Fixed

- **Unexpected task restart at large scale** - Disable AZ rebalancing for load test tasks and prevent replacement tasks from running load test
- **Missing `JVM_ARGS`** - Propagate base task definition env vars to load test tasks
...
- Upgrade Lambda runtime from Node.js 20 to Node.js 24 to address [Issue #277](https://github.com/aws-solutions/distributed-load-testing-on-aws/issues/277)
